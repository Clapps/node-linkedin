...
module.exports = class LinkedInRestClient {
  ...
  /**
   * Upload Rich Media
   * @link https://docs.microsoft.com/en-us/linkedin/marketing/integrations/community-management/shares/rich-media-shares#upload-rich-media
   *
   * @param body
   * @param access_token
   * @returns {Promise<Object>}
   */
  async createRichMedia(path, access_token) {
    if (!access_token) throw new Error('Access code cannot be empty');
    const url = `${this.url}/media/upload`;

    var formData = [
      { body: fs.createReadStream(path) }
    ];

    return this.invoke('POST', url, {}, undefined, { withAuth: true, access_token }, formData);
  }
  ...
  async getUserOrganizationsList(access_token) {
    if (!access_token) throw new Error('Access code cannot be empty');
    const url = `${this.url}/organizationalEntityAcls?q=roleAssignee&role=ADMINISTRATOR&state=APPROVED&projection=(elements*(*,roleAssignee~(localizedFirstName, localizedLastName), organizationalTarget~(id,localizedName,logoV2(original~:playableStreams))))`;
    return this.invoke('GET', url, undefined, undefined, { withAuth: true, access_token });
  }
   ...
   *
   * @param access_token, organization_urn
   * @returns {Promise<Object>}
   */
  async getUserOrganizationbyId(access_token, organization_urn) {
    if (!access_token) throw new Error('Access code cannot be empty');
    const url = `${this.url}/organizationalEntityAcls?q=organizationalTarget&organizationalTarget=${encodeURIComponent(organization_urn)}&role=ADMINISTRATOR&state=APPROVED&projection=(elements*(*,roleAssignee~(localizedFirstName, localizedLastName), organizationalTarget~(id,localizedName,logoV2(original~:playableStreams))))`;
    return this.invoke('GET', url, undefined, undefined, { withAuth: true, access_token });
  }
   ...
   */
  invoke(method, url, headers = { 'Content-Type': 'Application/json' }, body = {}, auth = { withAuth: false, access_token: null }, formData) {
    return new Promise((resolve, reject) => {
      let options;
      try {
        options = this.generateOptions(method, url, headers, body, auth, formData);
      } catch (err) {
        return reject(err);
      }
   ...
   * @param {object} formData - An object to pass to make a call which required authorization, example { withAuth: true, access_token: 'access_token' }
   *
   * @returns {object} The HttpOptions JSON object
   */
  generateOptions(method, url, headers, body, auth, formData) {
    headers['X-Restli-Protocol-Version'] = '2.0.0';
    if (auth.withAuth) {
      if (!auth.access_token) throw new Error('Missing required "access_token" in the auth body');
      headers['Authorization'] = `Bearer ${auth.access_token}`;
    }
    const options = { url, method, headers, preambleCRLF: true, postambleCRLF: true, };
    if (body) options['body'] = JSON.stringify(body);
    if (formData) options['multipart'] = formData;
    return options;
  }
};
